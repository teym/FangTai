	...
	var init = {
		...
		event : function(){
            $(window).bind("resize",function(){
                ...
                $("#showdata").css({width:$(window).width(),height:$quantum+($val>0?$val:0)});
            }).trigger("resize");

            var data = [12,15,30,55,26,18,15];
            var yearData = [18,15,18,15,12,15,12,19,20,13,17,25];
            var monthData = [18,15,18,15,12,15,12,18,15,18,15,12,15,12,19,20,13,17,25,12,19,20,13,17,25,12,19,20,13,17,25,18];          //时间切换
            ...
            $("body").on("touchstart",".icon-boy,.icon-girl",function(){
                if($(this).hasClass("icon-boy")&&$(".icon-set2").hasClass("active")){
...
var Canvas = function(id,param){
    ...
    function drawText(textVal,context,xVal,yVal,width,fontSize,fillStyle) {
        ...
        context.fillStyle = fillStyle?fillStyle:'RGBA(255,255,255,0.3)';
